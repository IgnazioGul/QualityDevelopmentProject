...
class WeatherCall:
    ...
    def check_key() -> str | TypeError:
        ...
        """
        response = WeatherCall.check(
            os.getenv(constants.get('KEY', 'WEATHER_KEY_NAME')))
        if response == '401':
    ...
    def get_coordinates(city: str, limit: int) -> dict | TypeError:
        ...
        """
        response = WeatherCall.geo_response(
            os.getenv(constants.get('KEY', 'WEATHER_KEY_NAME')), city, limit)
        if "cod" in response:
    ...
    def get_weather(lat: float, lon: float) -> dict | TypeError:
        ...
        """
        response = WeatherCall.weather_response(
            os.getenv(constants.get('KEY', 'WEATHER_KEY_NAME')), lat, lon)
    ...
    def check(key: str) -> str:
        return requests.get(constants.get('CONSTANTS', 'OPEN_WEATHER_CHECK') + key, timeout=10).json()['cod']

    @staticmethod
    def geo_response(key: str, city: str, limit: int) -> dict:
        return requests.get(f"{constants.get('CONSTANTS','OPEN_WEATHER_GEO')}q={city}&limit={limit}&appid={key}", timeout=10).json()

    @staticmethod
    def weather_response(key: str, lat: float, lon: float) -> dict:
        return requests.get(f"{constants.get('CONSTANTS','OPEN_WEATHER_CITY')}lat={lat}&lon={lon}&units=metric&appid={key}", timeout=10).json()
